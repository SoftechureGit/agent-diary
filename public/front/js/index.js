...
{
	 var result = confirm("Are you sure you want to delete this property from your wishlist?");
    if (result) {
        $('#ajaxLoader').show();
        $.ajax({
            headers: {
...
{ 
  ...
  if(val == "Release")
  { 
  	var result = confirm("Are you sure you want to release this property?");
  ...
  if(val == "Book")
  {
  	var result = confirm("Are you sure you want to book this property?");
  	 if (result) {
     window.open(BASE_URL +'/get-booking-form/'+id,'_blank');
  	 }
  }
}

function final_release()
{   var id=$("#inv_id").val();
...
function set_action_backup(id,val)
{ 
  if(val == "Release")
  { 
    var result = confirm("Are you sure you want to release this property?");
