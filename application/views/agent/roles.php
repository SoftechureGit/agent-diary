                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                ...
                                <!--<a href="<?= base_url(AGENT_URL.'role-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a>-->
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(AGENT_URL.'api/role_list') ?>'
      },
      'columns': [
         //{ data: 'role_id' },
         ...
         { 
          ...
          "render": function (data, type, row) {
            return "<a href='<?= base_url(AGENT_URL.'role-permission/') ?>"+row.role_id+"'><button class='btn btn-info btn-sm'><i class='fa fa-edit'></i> Set Permission</button></a>";
          }
         },
         { 
            className: "text-center",
           "render": function (data, type, row) {
         ...
         },
	        {
				...
				className: "text-center",
				"render": function (data, type, row) {
					return "<?php if(isset($menu_item_array['roles']) && $menu_item_array['roles']['rr_edit']) { ?><!--<a href='<?= base_url(AGENT_URL.'role-detail/') ?>"+row.role_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.role_id+",2)'><i class='fa fa-edit'></i></button><?php } ?> <?php if(isset($menu_item_array['roles']) && $menu_item_array['roles']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.role_id+")'><i class='fa fa-trash'></i></button><?php } ?>";
				}
...
function get_role(id) {
	 $.ajax({
        type: "POST",
        url: "<?= base_url(AGENT_URL.'api/get_role') ?>",
        data: {id:id},
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(AGENT_URL.'api/role_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_role(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(AGENT_URL.'api/delete_role'); ?>",
    data: {id:id},
