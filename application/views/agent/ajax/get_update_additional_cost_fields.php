<div class="row">
  ...
  <div class="form-group col-md-6">
    ...
    <select class="form-control" id="cost_type" name="cost_type" required="" onchange="getBasicCostData()">
      <option value="">Select Additional Cost</option>
      <?php if($product_data->parking_open) { ?>
      <option value="Parking_Open">Parking Open</option>
      <?php } ?>
      <?php if($product_data->parking_stilt) { ?>
      <option value="Parking_Stilt">Parking Stilt</option>
      <?php } ?>
      <?php if($product_data->parking_basment) { ?>
      <option value="Parking_Basment">Parking Basment</option>
      <?php } ?>
      <?php foreach ($additional_details as $item) { ?>
      <option value="Additional_<?= $item->product_additional_detail_id ?>"><?= ($item->price_component_name)?$item->price_component_name:'' ?> - Additional</option>
      <?php } ?>
      <?php foreach ($plc_details as $item) { ?>
      <option value="PLC_<?= $item->product_plc_detail_id ?>"><?= ($item->price_component_name)?$item->price_component_name:'' ?> - PLC</option>
      <?php } ?>
    </select>
  </div>
  <!--<div class="form-group col-md-6">
  ...
  <div class="form-group col-md-6 new_rate" style="display: none;">
    <label>New Rate:</label>
    <div class="row">
      <div class="col-md-4">
        <input type="text" class="form-control" placeholder="Enter Rate" id="new_rate" name="new_rate" required="">
      </div>
      <div class="col-md-4">
      ...
      <div class="col-md-4">
        <input type="text" class="form-control" placeholder="Enter GST" id="new_rate_gst" name="new_rate_gst">
      </div>
    </div>
  </div>
</div>
