               ...
               <div class="card-body">
                  <div class="row">
                     <div class="col-md-6 col-sm-6 col-xs-6">
                        <h4 class="card-title">Update Basic Cost</h4>
                     </div>
                  </div>
...
$("#inventory-form").validate({
    ...
    submitHandler: function(form) {
      ...
      var chk_ids = $('.chk_selection:checked').map(function() {return this.value;}).get().join(',');
      if (chk_ids=="") {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(AGENT_URL.'/api/update_inventory_status') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
...
function selectProject() {
   ...
   var product_id = $("#product_id").val();
    ...
    $.ajax({
         type: "POST",
         url: "<?= base_url(AGENT_URL.'api/get_update_inventory_status_fields') ?>",
...
function getBasicCostData(f='') {
    ...
    if (f=='') { 
        $("#new_inventory_status").val(""); 
    }
    $(".inventory-btn, #new_inventory_status").prop('disabled',true);
    $(".inventory_unit_list").html("");

   $.ajax({
         type: "POST",
         url: "<?= base_url(AGENT_URL.'api/get_update_inventory_status_data') ?>",
         ...
         success: function (response) {
           ...
           try {
             ...
             if (obj.status=='success') {
              ...
              $(".inventory-btn, #new_inventory_status").prop('disabled',false);
               ...
               for (var i = 0; i<items.length; i++) {
                var row = items[i];
                 ...
                 if (is=="1") {
                    inventory_status_name = "<span class='label label-pill label-info'>"+row.inventory_status_name+"</span>";
                 }
                 else if (is=="2") {
                    inventory_status_name = "<span class='label label-pill label-danger'>"+row.inventory_status_name+"</span>";
                 }
                 else if (is=="3") {
                    inventory_status_name = "<span class='label label-pill label-warning'>"+row.inventory_status_name+"</span>";
                 }
                 else if (is=="4") {
                    inventory_status_name = "<span class='label label-pill label-success'>"+row.inventory_status_name+"</span>";
                 }
                 row_html += "<tr>"+
                    "<td class='text-center'> "+ds+" <input type='hidden' name='in_unit["+row.inventory_id+"][unit_id]' value='"+row.inventory_id+"' /></td>"+
