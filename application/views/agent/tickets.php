                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                ...
                                <!--<a href="<?= base_url(AGENT_URL.'ticket-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a>-->
...
<script type="text/javascript">
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(AGENT_URL.'api/ticket_list') ?>'
      },
      'columns': [
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['tickets']) && $menu_item_array['tickets']['rr_edit']) { ?><a href='<?= base_url(AGENT_URL.'ticket-detail/') ?>"+row.ticket_track_id+"'><button type='button' class='btn btn-dark btn-sm'><i class='fa fa-eye'></i></button></a><!--<button type='button' class='btn btn-info btn-sm' onclick='formModal("+row.ticket_id+",2)'><i class='fa fa-eye'></i></button>--><?php //} ?> <?php //if(isset($menu_item_array['tickets']) && $menu_item_array['tickets']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.ticket_id+")'><i class='fa fa-trash'></i></button><?php //} ?>";
                }
...
function get_ticket(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(AGENT_URL.'api/get_ticket') ?>",
        data: {id:id},
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(AGENT_URL.'api/ticket_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_ticket(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(AGENT_URL.'api/delete_ticket'); ?>",
    data: {id:id},
