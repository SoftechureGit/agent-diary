                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Builder Groups</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['builder_groups']) && $menu_item_array['builder_groups']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'builder_group-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" builder_group="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" builder_group="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="builder_group_name" class="col-form-label">Builder Group Name:</label>
                        <input type="text" class="form-control" id="builder_group_name" name="builder_group_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="builder_group_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="builder_group_status" name="builder_group_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/builder_group_list') ?>'
      },
      'columns': [
         { data: 'builder_group_id' },
         //{ render: function (data, type, row, meta) { return meta.row + meta.settings._iDisplayStart + 1; } },
         { data: 'builder_group_name' },
         { 
           className: "text-center",
           "render": function (data, type, row) {
 
                if (row.builder_group_status === '1') {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['builder_groups']) && $menu_item_array['builder_groups']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'builder_group-detail/') ?>"+row.builder_group_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.builder_group_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['builder_groups']) && $menu_item_array['builder_groups']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.builder_group_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Builder Group');
        $(".form-btn").text('Add New');
    }
    else {
        get_builder_group(id);
    }
 }

function get_builder_group(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_builder_group') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.builder_group_id);
                    $("#builder_group_name").val(record.builder_group_name);
                    $("#builder_group_status").val(record.builder_group_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Builder Group');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/builder_group_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
    swal({
          ...
          if (isConfirm) {
            ...
            delete_builder_group(id)
          } 
        });
  }

  function delete_builder_group(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_builder_group'); ?>",
    data: {id:id},
