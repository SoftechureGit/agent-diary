                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['specifications']) && $menu_item_array['specifications']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'specification-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" specification="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" specification="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="specification_name" class="col-form-label">Specification Name:</label>
                        <input type="text" class="form-control" id="specification_name" name="specification_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="specification_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="specification_status" name="specification_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/specification_list') ?>'
      },
      'columns': [
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['specifications']) && $menu_item_array['specifications']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'specification-detail/') ?>"+row.specification_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.specification_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['specifications']) && $menu_item_array['specifications']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.specification_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Specification');
...
function get_specification(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_specification') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.specification_id);
                    $("#specification_name").val(record.specification_name);
                    $("#specification_status").val(record.specification_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Specification');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/specification_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_specification(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_specification'); ?>",
    data: {id:id},
