                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Firm Types</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['firm_types']) && $menu_item_array['firm_types']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'firm_type-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" firm_type="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" firm_type="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="firm_type_name" class="col-form-label">Firm Type Name:</label>
                        <input type="text" class="form-control" id="firm_type_name" name="firm_type_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="firm_type_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="firm_type_status" name="firm_type_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/firm_type_list') ?>'
      },
      'columns': [
         { data: 'firm_type_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['firm_types']) && $menu_item_array['firm_types']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'firm_type-detail/') ?>"+row.firm_type_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.firm_type_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['firm_types']) && $menu_item_array['firm_types']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.firm_type_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Firm Type');
...
function get_firm_type(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_firm_type') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.firm_type_id);
                    $("#firm_type_name").val(record.firm_type_name);
                    $("#firm_type_status").val(record.firm_type_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Firm Type');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/firm_type_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_firm_type(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_firm_type'); ?>",
    data: {id:id},
