                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Construction Age</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['construction_ages']) && $menu_item_array['construction_ages']['rr_create']) { ?>
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" construction_age="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" construction_age="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="construction_age_name" class="col-form-label">Construction Age:</label>
                        <input type="text" class="form-control" id="construction_age_name" name="construction_age_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="construction_age_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="construction_age_status" name="construction_age_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/construction_age_list') ?>'
      },
      'columns': [
         { data: 'construction_age_id' },
         //{ render: function (data, type, row, meta) { return meta.row + meta.settings._iDisplayStart + 1; } },
         { data: 'construction_age_name' },
         { 
           className: "text-center",
           "render": function (data, type, row) {
 
                if (row.construction_age_status === '1') {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['construction_ages']) && $menu_item_array['construction_ages']['rr_edit']) { ?><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.construction_age_id+",2)'><i class='fa fa-edit'></i></button><?php //} ?> <?php //if(isset($menu_item_array['construction_ages']) && $menu_item_array['construction_ages']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.construction_age_id+")'><i class='fa fa-trash'></i></button><?php //} ?>";
                }
            }
        ],
        'aoColumnDefs': [
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Construction Age');
        $(".form-btn").text('Add New');
    }
    else {
        get_construction_age(id);
    }
 }

function get_construction_age(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_construction_age') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.construction_age_id);
                    $("#construction_age_name").val(record.construction_age_name);
                    $("#construction_age_status").val(record.construction_age_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Construction Age');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/construction_age_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
    swal({
          ...
          if (isConfirm) {
            ...
            delete_construction_age(id)
          } 
        });
  }

  function delete_construction_age(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_construction_age'); ?>",
    data: {id:id},
