                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['designations']) && $menu_item_array['designations']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'designation-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" designation="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" designation="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="designation_name" class="col-form-label">Designation:</label>
                        <input type="text" class="form-control" id="designation_name" name="designation_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="designation_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="designation_status" name="designation_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/designation_list') ?>'
      },
      'columns': [
         { data: 'designation_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['designations']) && $menu_item_array['designations']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'designation-detail/') ?>"+row.designation_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.designation_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['designations']) && $menu_item_array['designations']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.designation_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
...
function get_designation(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_designation') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.designation_id);
                    $("#designation_name").val(record.designation_name);
                    $("#designation_status").val(record.designation_status);
                    $(".error-msg").html('');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/designation_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_designation(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_designation'); ?>",
    data: {id:id},
