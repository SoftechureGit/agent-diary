                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Facings</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['facings']) && $menu_item_array['facings']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'facing-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" facing="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" facing="document">
        <div class="modal-content">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="facing_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="facing_status" name="facing_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/facing_list') ?>'
      },
      'columns': [
         { data: 'facing_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['facings']) && $menu_item_array['facings']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'facing-detail/') ?>"+row.facing_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.facing_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['facings']) && $menu_item_array['facings']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.facing_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Facing');
...
function get_facing(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_facing') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.facing_id);
                    $("#title").val(record.title);
                    $("#facing_status").val(record.facing_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Facing');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/facing_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_facing(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_facing'); ?>",
    data: {id:id},
