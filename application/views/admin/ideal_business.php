                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Ideal Business</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['ideal_businesss']) && $menu_item_array['ideal_businesss']['rr_create']) { ?>
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" ideal_business="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" ideal_business="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="ideal_business_name" class="col-form-label">Name:</label>
                        <input type="text" class="form-control" id="ideal_business_name" name="ideal_business_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="ideal_business_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="ideal_business_status" name="ideal_business_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/ideal_business_list') ?>'
      },
      'columns': [
         { data: 'ideal_business_id' },
         //{ render: function (data, type, row, meta) { return meta.row + meta.settings._iDisplayStart + 1; } },
         { data: 'ideal_business_name' },
         { 
           className: "text-center",
           "render": function (data, type, row) {
 
                if (row.ideal_business_status === '1') {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['ideal_businesss']) && $menu_item_array['ideal_businesss']['rr_edit']) { ?><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.ideal_business_id+",2)'><i class='fa fa-edit'></i></button><?php //} ?> <?php //if(isset($menu_item_array['ideal_businesss']) && $menu_item_array['ideal_businesss']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.ideal_business_id+")'><i class='fa fa-trash'></i></button><?php //} ?>";
                }
            }
        ],
        'aoColumnDefs': [
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Ideal Business');
        $(".form-btn").text('Add New');
    }
    else {
        get_ideal_business(id);
    }
 }

function get_ideal_business(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_ideal_business') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.ideal_business_id);
                    $("#ideal_business_name").val(record.ideal_business_name);
                    $("#ideal_business_status").val(record.ideal_business_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Ideal Business');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/ideal_business_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
    swal({
          ...
          if (isConfirm) {
            ...
            delete_ideal_business(id)
          } 
        });
  }

  function delete_ideal_business(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_ideal_business'); ?>",
    data: {id:id},
