                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['lead_sources']) && $menu_item_array['lead_sources']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'lead_source-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" lead_source="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" lead_source="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="lead_source_name" class="col-form-label">Lead Source Name:</label>
                        <input type="text" class="form-control" id="lead_source_name" name="lead_source_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="lead_source_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="lead_source_status" name="lead_source_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/lead_source_list') ?>'
      },
      'columns': [
         { data: 'lead_source_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['lead_sources']) && $menu_item_array['lead_sources']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'lead_source-detail/') ?>"+row.lead_source_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.lead_source_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['lead_sources']) && $menu_item_array['lead_sources']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.lead_source_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Lead Source');
...
function get_lead_source(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_lead_source') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.lead_source_id);
                    $("#lead_source_name").val(record.lead_source_name);
                    $("#lead_source_status").val(record.lead_source_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Lead Source');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/lead_source_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_lead_source(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_lead_source'); ?>",
    data: {id:id},
