                        ...
                        <div class="card login-form mb-0">
                            <div class="card-body pt-2 ">
                                <div class="text-center">
                                    <h3 style="margin-bottom: 20px;margin-top: 10px;">Welcome to <?= SITE_TITLE ?></h3>
                                </div>

                                <div class="error-msg"></div>

                                <form class="mb-1 login-input" id="login-form" method="post">
                                    <div class="form-group">
                                        <input type="text" class="form-control" placeholder="Username" id="username" name="username" value="<?= ($_SERVER['HTTP_HOST']=='localhost')?'admin@gmail.com':'' ?>">
                                    </div>
                                    <div class="form-group">
                                        <input type="password" class="form-control" placeholder="Password" id="password" name="password" value="<?= ($_SERVER['HTTP_HOST']=='localhost')?'12345678':'' ?>">
                                    </div>
                                    <button class="btn btn-dark submit w-100 form-btn">LOGIN</button>
                                </form>
                                <div class="row" style="padding: 8px 17px 0px 17px;">
                                 <div class="col-md-12 col-12 tar">
                                    <p class=" login-form__footer"><a href="javascript:void()" class="text-primary" style="color: #76838f !important;"> Forgot Password ?  </a> </p>
                                </div>
...
function alertMessage(type,message) {
 ...
 $("#login-form").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'/admin_login_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                  ...
                  window.location.href="<?= base_url(ADMIN_URL) ?>";
                }
                else {
