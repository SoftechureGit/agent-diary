                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                ...
                                <!--<a href="<?= base_url(ADMIN_URL.'amenitie-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" amenitie="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" amenitie="document">
            ...
            <form id="form-modal" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="amenitie_name" class="col-form-label">Amenitie Name:</label>
                        <input type="text" class="form-control" id="amenitie_name" name="amenitie_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="amenitie_name" class="col-form-label">Image:</label>
                        <input type="file" class="form-control" id="amenitie_image" name="amenitie_image" accept="image/*">

                        <img src="" style="width: 80px;height: 80px;margin-top: 15px;" id="amenitie_img" alt="Amenitie Image">
                    </div>
                    <div class="form-group">
                        <label for="amenitie_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="amenitie_status" name="amenitie_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/amenitie_list') ?>'
      },
      'columns': [
         { data: 'amenitie_id' },
         ...
         { className: "text-center",
           "render": function (data, type, row) {
                if (row.amenitie_image == null || row.amenitie_status == '') {
                    return "";
                }
                else {
                    return "<img src='<?= base_url('uploads/images/amenitie/') ?>"+row.amenitie_image+"' style='width: 50px;height: 50px;' alt='Amenitie Image'>";
                }
            }
             },
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<!--<a href='<?= base_url(ADMIN_URL.'amenitie-detail/') ?>"+row.amenitie_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.amenitie_id+",2)'><i class='fa fa-edit'></i></button>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.amenitie_id+")'><i class='fa fa-trash'></i></button>>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Amenitie');
...
function get_amenitie(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_amenitie') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.amenitie_id);
                    $("#amenitie_name").val(record.amenitie_name);
                    $("#amenitie_status").val(record.amenitie_status);

                    if (record.amenitie_image == null || record.amenitie_status == '') {
                     $("#amenitie_img").hide();
                    }
                    else {
                      $("#amenitie_img").attr("src","<?= base_url('uploads/images/amenitie/') ?>"+record.amenitie_image).show();
                    }
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Amenitie');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/amenitie_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_amenitie(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_amenitie'); ?>",
    data: {id:id},
