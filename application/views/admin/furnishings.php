                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Furnishings</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['furnishings']) && $menu_item_array['furnishings']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'furnishing-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
                            ...
                            <table class="table table-bordered" id="empTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Furnishing Name</th>
                                        <th class="nosort wd-100 text-center">Type</th>
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" furnishing="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" furnishing="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="furnishing_name" class="col-form-label">Furnishing Name:</label>
                        <input type="text" class="form-control" id="furnishing_name" name="furnishing_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="input_type" class="col-form-label">Type:</label>
                        <select class="form-control" id="input_type" name="input_type" required="">
                            <option value="">Select Type</option>
                            <option value="1">Input Number</option>
                            <option value="2">Yes/No</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="furnishing_status" class="col-form-label">Status:</label>
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/furnishing_list') ?>'
      },
      'columns': [
         { data: 'furnishing_id' },
         //{ render: function (data, type, row, meta) { return meta.row + meta.settings._iDisplayStart + 1; } },
         { data: 'furnishing_name' },
         { 
           className: "text-center",
           "render": function (data, type, row) {
 
                if (row.input_type === '1') {
                    return "<span class='label label-pill label-primary'>Input Number</span>";
                }
                else {
                    return "<span class='label label-pill label-info'>Yes/No</span>";
                }
            }
         },
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['furnishings']) && $menu_item_array['furnishings']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'furnishing-detail/') ?>"+row.furnishing_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.furnishing_id+",2)'><i class='fa fa-edit'></i></button><?php //} ?> <?php //if(isset($menu_item_array['furnishings']) && $menu_item_array['furnishings']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.furnishing_id+")'><i class='fa fa-trash'></i></button><?php //  } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Furnishing');
...
function get_furnishing(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_furnishing') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.furnishing_id);
                    $("#furnishing_name").val(record.furnishing_name);
                    $("#furnishing_status").val(record.furnishing_status);
                    $("#input_type").val(record.input_type);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Furnishing');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/furnishing_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
    swal({
          ...
          if (isConfirm) {
            ...
            delete_furnishing(id)
          } 
        });
  }

  function delete_furnishing(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_furnishing'); ?>",
    data: {id:id},
