...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" accomodation="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" accomodation="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="accomodation_name" class="col-form-label">Accomodation Name:</label>
                        <input type="text" class="form-control" id="accomodation_name" name="accomodation_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="accomodation_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="accomodation_status" name="accomodation_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/accomodation_list') ?>'
      },
      'columns': [
         { data: 'accomodation_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<!--<a href='<?= base_url(ADMIN_URL.'accomodation-detail/') ?>"+row.accomodation_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.accomodation_id+",2)'><i class='fa fa-edit'></i></button>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.accomodation_id+")'><i class='fa fa-trash'></i></button>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Accomodation');
...
function get_accomodation(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_accomodation') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.accomodation_id);
                    $("#accomodation_name").val(record.accomodation_name);
                    $("#accomodation_status").val(record.accomodation_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Accomodation');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/accomodation_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_accomodation(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_accomodation'); ?>",
    data: {id:id},
