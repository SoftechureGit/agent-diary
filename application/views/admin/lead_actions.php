                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Lead Actions</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['lead_actions']) && $menu_item_array['lead_actions']['rr_create']) { ?>
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" lead_action="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" lead_action="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="lead_action_name" class="col-form-label">Lead Action:</label>
                        <input type="text" class="form-control" id="lead_action_name" name="lead_action_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="lead_action_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="lead_action_status" name="lead_action_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/lead_action_list') ?>'
      },
      'columns': [
         { data: 'lead_action_id' },
         //{ render: function (data, type, row, meta) { return meta.row + meta.settings._iDisplayStart + 1; } },
         { data: 'lead_action_name' },
         { 
           className: "text-center",
           "render": function (data, type, row) {
 
                if (row.lead_action_status === '1') {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['lead_actions']) && $menu_item_array['lead_actions']['rr_edit']) { ?><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.lead_action_id+",2)'><i class='fa fa-edit'></i></button><?php //} ?> <?php //if(isset($menu_item_array['lead_actions']) && $menu_item_array['lead_actions']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.lead_action_id+")'><i class='fa fa-trash'></i></button><?php //} ?>";
                }
            }
        ],
        'aoColumnDefs': [
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Lead Action');
...
function get_lead_action(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_lead_action') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.lead_action_id);
                    $("#lead_action_name").val(record.lead_action_name);
                    $("#lead_action_status").val(record.lead_action_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Lead Action');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/lead_action_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
    swal({
          ...
          if (isConfirm) {
            ...
            delete_lead_action(id)
          } 
        });
  }

  function delete_lead_action(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_lead_action'); ?>",
    data: {id:id},
