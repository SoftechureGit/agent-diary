                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Lead Stages</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['lead_stages']) && $menu_item_array['lead_stages']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'lead_stage-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" lead_stage="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" lead_stage="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="lead_stage_name" class="col-form-label">Lead Stage Name:</label>
                        <input type="text" class="form-control" id="lead_stage_name" name="lead_stage_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="lead_stage_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="lead_stage_status" name="lead_stage_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/lead_stage_list') ?>'
      },
      'columns': [
         { data: 'lead_stage_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['lead_stages']) && $menu_item_array['lead_stages']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'lead_stage-detail/') ?>"+row.lead_stage_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.lead_stage_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['lead_stages']) && $menu_item_array['lead_stages']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.lead_stage_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Lead Stage');
...
function get_lead_stage(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_lead_stage') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.lead_stage_id);
                    $("#lead_stage_name").val(record.lead_stage_name);
                    $("#lead_stage_status").val(record.lead_stage_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Lead Stage');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/lead_stage_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_lead_stage(id) {

    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_lead_stage'); ?>",
    data: {id:id},
