                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 col-sm-6 col-xs-6">
                                <h4 class="card-title">Fiinances</h4>
                            </div>
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['finances']) && $menu_item_array['finances']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'finance-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" finance="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" finance="document">
            ...
            <form id="form-modal" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="finance_name" class="col-form-label">Fiinance Name:</label>
                        <input type="text" class="form-control" id="finance_name" name="finance_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="finance_name" class="col-form-label">Image:</label>
                        <input type="file" class="form-control" id="finance_image" name="finance_image" accept="image/*">

                        <img src="" style="width: 80px;height: 80px;margin-top: 15px;" id="finance_img" alt="Fiinance Image">
                    </div>
                    <div class="form-group">
                        <label for="finance_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="finance_status" name="finance_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/finance_list') ?>'
      },
      'columns': [
         { data: 'finance_id' },
         ...
         { className: "text-center",
           "render": function (data, type, row) {
                if (row.finance_image == null || row.finance_status == '') {
                    return "";
                }
                else {
                    return "<img src='<?= base_url('uploads/images/finance/') ?>"+row.finance_image+"' style='width: 50px;height: 50px;' alt='Fiinance Image'>";
                }
            }
             },
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['finances']) && $menu_item_array['finances']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'finance-detail/') ?>"+row.finance_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.finance_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['finances']) && $menu_item_array['finances']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.finance_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Fiinance');
...
function get_finance(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_finance') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.finance_id);
                    $("#finance_name").val(record.finance_name);
                    $("#finance_status").val(record.finance_status);

                    if (record.finance_image == null || record.finance_status == '') {
                     $("#finance_img").hide();
                    }
                    else {
                      $("#finance_img").attr("src","<?= base_url('uploads/images/finance/') ?>"+record.finance_image).show();
                    }
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Fiinance');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/finance_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_finance(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_finance'); ?>",
    data: {id:id},
