                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php // if(isset($menu_item_array['property_types']) && $menu_item_array['property_types']['rr_create']) { ?>
                                <!--<a href="<?= base_url(ADMIN_URL.'property_type-detail') ?>"><button type="button" class="btn btn-info btn-sm" >Add New</button></a><?php //  } ?>-->
...
<!-- modal form -->
<div class="modal fade" id="formModal" tabindex="-1" property_type="dialog" aria-labelledby="formModalLabel" aria-hidden="true">
    <div class="modal-dialog" property_type="document">
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="property_type_name" class="col-form-label">Property Type Name:</label>
                        <input type="text" class="form-control" id="property_type_name" name="property_type_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="property_type_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="property_type_status" name="property_type_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/property_type_list') ?>'
      },
      'columns': [
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php // if(isset($menu_item_array['property_types']) && $menu_item_array['property_types']['rr_edit']) { ?><!--<a href='<?= base_url(ADMIN_URL.'property_type-detail/') ?>"+row.property_type_id+"'><button type='button' class='btn btn-success btn-sm'><i class='fa fa-edit'></i></button></a><?php //  } ?>--><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.property_type_id+",2)'><i class='fa fa-edit'></i></button><?php // } ?> <?php // if(isset($menu_item_array['property_types']) && $menu_item_array['property_types']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.property_type_id+")'><i class='fa fa-trash'></i></button><?php // } ?>";
                }
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Property Type');
...
function get_property_type(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_property_type') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.property_type_id);
                    $("#property_type_name").val(record.property_type_name);
                    $("#property_type_status").val(record.property_type_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Property Type');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/property_type_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_property_type(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_property_type'); ?>",
    data: {id:id},
