                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-6" align="right">
                                <?php //if(isset($menu_item_array['budgets']) && $menu_item_array['budgets']['rr_create']) { ?>
            ...
            <form id="form-modal" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="budget_name" class="col-form-label">Budget:</label>
                        <input type="text" class="form-control" id="budget_name" name="budget_name" required="">
                    </div>
                    <div class="form-group">
                        <label for="budget_status" class="col-form-label">Status:</label>
                        <select class="form-control" id="budget_status" name="budget_status" required="">
...
<script type="text/javascript">
    ...
    var table=$('#empTable').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
         'url':'<?= base_url(ADMIN_URL.'api/budget_list') ?>'
      },
      'columns': [
         { data: 'budget_id' },
         ...
         { 
           ...
           "render": function (data, type, row) {
         ...
         },
            {
                ...
                className: "text-center",
                "render": function (data, type, row) {
                    return "<?php //if(isset($menu_item_array['budgets']) && $menu_item_array['budgets']['rr_edit']) { ?><button type='button' class='btn btn-success btn-sm' onclick='formModal("+row.budget_id+",2)'><i class='fa fa-edit'></i></button><?php //} ?> <?php //if(isset($menu_item_array['budgets']) && $menu_item_array['budgets']['rr_delete']) { ?>&nbsp;&nbsp;<button type='button' class='btn btn-danger btn-sm' onclick='confirDelete("+row.budget_id+")'><i class='fa fa-trash'></i></button><?php //} ?>";
                }
            }
        ],
        'aoColumnDefs': [
 ...
 function formModal(id,type) {
    ...
    if (type==1) {
        ...
        $("#formModalLabel").text('Add New Budget');
...
function get_budget(id) {
     $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/get_budget') ?>",
        data: {id:id},
            ...
            var obj;
              try {
                ...
                if (obj.status=='success') {
                    ...
                    $("#fid").val(record.budget_id);
                    $("#budget_name").val(record.budget_name);
                    $("#budget_status").val(record.budget_status);
                    $(".error-msg").html('');

                    $("#formModalLabel").text('Edit Budget');
...
function alertMessage(type,message) {
 ...
 $("#form-modal").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url(ADMIN_URL.'api/budget_process') ?>",
        data: fd,
        cache: false,
        processData: false,
        contentType: false,
 ...
 function confirDelete(id) {
  ...
  function delete_budget(id) {
    ...
    $.ajax({
    type: "POST",
    url: "<?php echo base_url(ADMIN_URL.'api/delete_budget'); ?>",
    data: {id:id},
