...
class Api extends CI_Controller
{
    ...
    public function dashboard()
    {
        ...
        # FCM Update
        if($fcm_device_id && $user_id):
            $this->db->where('fcm_device_id', $fcm_device_id)->update('tbl_users', [ 'fcm_device_id' => null ]);
            $this->db->where('user_id',  $user_id)->update('tbl_users', [ 'fcm_device_id' => $fcm_device_id ]);
        endif;
        # End FCM Update
    ...
    public function get_lead_filter_default_data()
    {
        ...
        if ($this->input->post()) {
            ...
            $array['data'] = array(
                ...
                "designation_list" => $lead_designation_list,
                "product_type_list" => $product_types_list,
                "lead_option_list" => $lead_option_list,
                "product_unit_type_list" => $product_unit_type_list,
                "lead_action_list" => $lead_action_list,
                "product_list" => $product_list,
                "product_listing" => $listing_type_list,
                "facing_list" => $facing_list,
                "furnishing_status_list" => $furnised_status_list,
                "business_ideals" => $ideal_business_list,
                "furnishing_list" => $furnishing_list,
                "construction_age" => $construction_age_list,
                "amenities" => $amenitie_list
            );
        } else {
            $array['data'] = array('status' => 'false', 'msg' => 'Some error occurred, please try again.');
        }
            ...
            foreach ($transfer_lead_ids as  $transfer_lead_id) {
                ...
                *******************************************************************************/ 
                $fcm_notify_data      =   (object) [ 
                                                            ...
                                                            'message'   => "Agent has assign you new lead ",
                                                        ];
    ...
    public function teams()
    {
        ...
        $select = "CONCAT(
                            ...
                            user.email,
                            user.mobile,
                            user.date_register as register_date,
                            role.role_name,
                            CASE
                                WHEN user.user_status = 0 THEN 'Deactive'
                                WHEN user.user_status = 1 THEN 'Active'
                                ELSE 'N/A'
                            END AS status_label
